...
- **The memlog is the process memory.** Every decision, source batch, load-bearing claim, plan change, and assumption is one append-only line, always through the script: `uv run {project-root}/_agent-workflows/scripts/memlog.py` with `--type <decision|source|claim|assumption|question|event>`.
- Web access is required for Run. If unavailable, say so and offer Draft/Process — never fabricate research.

## Date and freshness preflight

Before research begins, obtain the actual runtime date from the environment and set the run's `as_of` date. Include that date when querying current packages, tools, APIs, market conditions, or other time-sensitive subjects. Resolve exact versions and release/publication dates from the source rather than assuming a remembered version is latest. Record `as_of`, source publication/release date, and retrieval date in `.memlog.md` and carry them into `research.md`. On refresh or deepen, date new evidence separately and do not relabel older evidence as current.

## Evidence tool routing

- Use `context7-mcp` when a research question depends on current documentation for a named library, framework, SDK, or API.
- Use `opensrc` when types or documentation do not explain a dependency's actual implementation behavior.
- Use `gh` for primary GitHub repositories, releases, issues, and changelogs when they are relevant evidence.
- Use the configured web-research path for market, domain, competitive, user-voice, and academic claims. Use `playwright-mcp` only when observing a live web application is part of the question.
- Store source material in `imports/`, filtered evidence in `digests/`, and the claim plus source pointer in `.memlog.md` before incorporating it into `research.md`.
...
1. Resolve customization: `{skill-root}/customize.toml` (on failure read `{skill-root}/customize.toml`, use defaults). Run `{workflow.activation_steps_prepend}`, then `{workflow.activation_steps_append}`.
...
3. Resolve exactly one active initiative from `core.runs_root`; multiple active runs require explicit selection and a missing run blocks before writing. Bind research to the selected run's `01-research/` directory and use the topic slug in each child folder/file. If a run folder for this topic already exists there, offer to resume or extend it rather than start a duplicate. Register durable research artifacts with `run_state.py`.
