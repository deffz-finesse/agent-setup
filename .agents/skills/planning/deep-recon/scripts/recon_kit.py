...
# ///
"""recon_kit — deterministic helpers for deep-recon.
...
def parse_date(raw: str) -> date:
    ...
    for fmt in ("%Y-%m-%d", "%Y-%m", "%Y"):
        try:
            return datetime.strptime(raw, fmt).replace(tzinfo=timezone.utc).date()  # noqa: UP017
        except ValueError:
            continue
...
def cmd_staleness(args) -> int:
    try:
        ...
        today = parse_date(args.today) if args.today else datetime.now(timezone.utc).date()  # noqa: UP017
...
def cmd_slug(args) -> int:
    ...
    folder = (args.pattern
              ...
              .replace(
                  "{date}",
                  args.date or datetime.now(timezone.utc).date().isoformat(),  # noqa: UP017
              ))
