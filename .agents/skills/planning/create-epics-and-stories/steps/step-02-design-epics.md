...
- 💾 Update {{epics_list}} in `{epics_output_path}/index.md` and write each approved epic to its own shard.
...
Load `{epics_output_path}/index.md` and `{epics_output_path}/requirements-inventory.md` and review:
...
5. **Coarse by Design**: Record outcomes and risk boundaries, not a complete story sequence or acceptance criteria for distant work
6. **Implementation Efficiency**: Consider consolidating horizons that would repeatedly change the same core area without a useful feedback boundary
...
- Each epic describes a coherent outcome horizon
...
4. **Dependencies and Risks**: What must be learned or exist before this horizon is refined
...
"Do you approve this rough epic structure for identifying candidate increments?"
...
After approval, update the inventory and index, then create one shard per approved epic from `../templates/epic-template.md`:

1. Replace `{{requirements_coverage_map}}` in `requirements-inventory.md` with the coverage map.
2. Replace `{{epics_list}}` in `index.md` with ordered links to the approved `epic-<NN>-<slug>.md` shards and a one-line goal for each.
3. Seed each shard from `epic-template.md`, replacing its epic number, title, goal, and requirements-covered fields; leave candidate increments and risks for Step 3.
4. Ensure all FRs are mapped to epics.
...
- IF C: Save approved epics_list to `{epics_output_path}/index.md`, update frontmatter, and write epic shards before reading fully and following: ./step-03-create-stories.md
...
ONLY WHEN C is selected and the approved epics_list is saved to document, will you then read fully and follow: ./step-03-create-stories.md to identify coarse candidate increments.
