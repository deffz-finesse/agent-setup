...
At the opening greeting, let the user know they can invoke `advanced-elicitation` for deeper exploration at any point.
...
When the brief includes current market, domain, or feasibility claims, use `deep-recon` for research; use `context7-mcp` for named technology/API documentation, `opensrc` for dependency internals, and `gh` for primary repository or release evidence. Persist source imports and evidence digests under `<run-dir>/01-research/product-brief/`, register them, and link the brief to them. Do not hide research inside a planning addendum or conversation-only summary.

Obtain the runtime date before making time-sensitive claims and record it as the brief's `as_of` date. Preserve source publication/release dates separately from the retrieval date, especially when describing current products, packages, or market conditions.

Resolve the run once: discover `core.runs_root` from the central config and inspect each
`run.yaml` below it. Keep only manifests with `status: active`; if more than one exists,
ask which run owns this brief. If none exists, Create is the initiative-starting path:
offer to initialize a named run with `run_state.py init --project-root {project-root}
--title <title> --slug <slug>`, then use the newly-created run. In headless mode, a
missing or ambiguous title/slug blocks without creating a run. Update and Validate
require an explicit existing run directory. Never fall back to a standalone artifact
root or pass the phrase `the current project` as live data; resolve a mechanical
`{project_slug}` from the run title/slug.
...
**Create.** A brief the user is proud of, that meets their needs, drawn out through real conversation — do not assume: instead converse and understand, and then help craft the best product brief for their needs. Begin in `## Discovery` before drafting; the brief comes after the picture is on the table. Shape follows the product and need. Treat `{workflow.brief_template}` as a starting structure, not a contract: drop sections that do not earn their place, add sections the product needs, reorder freely - create sections for specialized domains or concerns also as needed. The brief serves the product's story, not the template's shape. Bind `{doc_workspace}` to `<run-dir>/02-plan/`, `{memlog_workspace}` to `<run-dir>/00-discovery/product-brief/`, and `{research_workspace}` to `<run-dir>/01-research/product-brief/`. Write `brief.md` and seed discovery memory with `uv run {project-root}/_agent-workflows/scripts/memlog.py init --workspace {memlog_workspace} --field topic="<product>"`. Register both `02-plan/brief.md` as a draft product brief and `00-discovery/product-brief/.memlog.md` as current discovery memory with `run_state.py add-artifact`; register any research files in `01-research/` the same way. If a gate fails, stop before writing and surface the blocker. Update and Validate use the same three workspaces.

**Update.** Reconcile an existing brief with a change signal. Before proposing changes, read the brief, addendum, `.memlog.md`, and original inputs — and run the `## Discovery` posture against the change signal (a patch applied without context becomes drift). If `.memlog.md` is missing (a legacy or pre-standard brief), init it with `uv run {project-root}/_agent-workflows/scripts/memlog.py init --workspace {memlog_workspace}` first — this update is its first entry. Surface conflicts with prior decisions before changing. Headless override: log the reversal via `uv run {project-root}/_agent-workflows/scripts/memlog.py append --workspace {memlog_workspace} --type override --text "<reversal + rationale>"`, then apply; halt `blocked` if intent is ambiguous. If the change is fundamental, offer Create instead of patching.
...
```json
{
  ...
  "addendum": "{doc_workspace}/brief-addendum.md",
  "memlog": "{memlog_workspace}/.memlog.md",
  "open_questions": [],
  "external_handoffs": [
...
- **Persistence is real-time.** Once Create intent is confirmed, `brief.md`, the discovery memlog, and any research workspace exist on disk and the user knows their phase-owned paths.
- **File roles.** `00-discovery/product-brief/.memlog.md` is the canonical discovery memory and audit trail — every decision, change, and override lands as one append-only line through `memlog.py`. `01-research/product-brief/` owns source imports and evidence digests. `02-plan/brief.md` is the rough planning output. `brief-addendum.md` may preserve user-contributed planning depth that does not fit the brief, but it must not become a substitute research store.
...
1. Memlog audit + addendum review: the user ends this step with an explicit, shared accounting of how the meaningful contents of `.memlog.md` were handled — captured in the brief, captured in `brief-addendum.md` (which may already hold detail captured during the conversation — see `## Constraints` for what belongs there), or set aside as process noise.
2. Polish: apply each entry in `{workflow.doc_standards}` (a `skill:`, `file:`, or plain-text directive) to `brief.md` (and `brief-addendum.md` if it exists). Run passes as parallel subagents - apply all doc standards to `brief.md` first, then `brief-addendum.md` so we present a high-quality draft for the user to review and finalize.
...
4. Set `status: current` in `brief.md`'s frontmatter; re-register the brief,
   discovery memlog, planning addendum when present, and every research import or
   digest in its owning phase. Then advance the selected run with
   `run_state.py set-phase --phase overall-plan`.
5. Tell the user it is ready: local paths and external destinations (URLs returned from handoffs). Invoke `help` to suggest what next steps make sense in the workflow ecosystem.
