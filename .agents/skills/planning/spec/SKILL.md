...
1. Resolve customization: `{skill-root}/customize.toml`. On failure, read `{skill-root}/customize.toml` directly.
2. Run `{workflow.activation_steps_prepend}`. Treat `{workflow.persistent_facts}` as foundational context (`file:` entries are loaded).
3. Resolve config: `uv run {project-root}/_agent-workflows/scripts/resolve_config.py --project-root {project-root}`. The single `_agent-workflows/_config/config.toml` file provides any project settings; missing identity and language values use runtime defaults.

4. Resolve the active initiative from `core.runs_root` and its `run.yaml`. A Create with no active run is offered only by `product-brief` or `prd`; otherwise stop and report that an initiative run is required. If more than one run is active, require an explicit run selection. Bind this workflow to the selected run's `02-plan/spec/` directory, and derive `{project_slug}` mechanically from the run slug or the requested spec slug. Never write a standalone spec folder under a legacy root.

When the contract depends on current external behavior, load `context7-mcp` for named library, framework, SDK, or API documentation; load `opensrc` for dependency internals; and use `gh` for primary repository or release evidence. Record load-bearing sources, versions, and retrieval dates in the memlog or an appropriate companion before deriving `SPEC.md`.

At the start of a run that makes time-sensitive claims, obtain the runtime date and record it as `as_of`. Verify package/API/platform versions against that date, record exact versions and retrieval dates, and preserve older evidence dates on resumed runs.
...
The spec is **always a folder** at `<run-dir>/02-plan/spec/` inside the selected initiative run. Register `SPEC.md`, every spec-authored companion, `stories.yaml` when present, and `.memlog.md` with `run_state.py add-artifact`, using `draft` while work is resumable and `current` only after self-validation. Review scratch belongs under `<run-dir>/02-plan/spec/reviews/` and must use the requested target slug/date. There is no standalone fallback.
...
- Same slug in the selected run updates the existing folder in place, preserving capability IDs. A different slug must use a distinct child target or a new run; do not silently overwrite an unrelated spec.
...
Deriving the contract from a living log instead of editing the contract in place is what lets the steps around the spec (PRD, UX, architecture, epics) run in any order and feed the same spec without merge drift: the log only accumulates, the artifact is re-rendered. So the spec is updated _only_ by re-deriving it here — spec is its single writer; a hand-edit to `SPEC.md` from outside is unsupported and is overwritten on the next derive.

Writes go through the shared script — `{project-root}/_agent-workflows/scripts/memlog.py` (atomic; never read it back except to resume):

- `uv run {project-root}/_agent-workflows/scripts/memlog.py init --workspace {spec-folder} --field topic="<what is being specced>"` — once, at create.
- `uv run {project-root}/_agent-workflows/scripts/memlog.py append --workspace {spec-folder} --type <decision|constraint|capability|assumption|question|direction|note|event> --text "<one-line gist, reason included>"` — as each lands.
...
A recognized domain implication the input leaves unaddressed _is_ such a gap — name it as an `open_questions[]` entry (healthcare input silent on PHI/HIPAA, payments silent on PCI, control systems silent on fail-safe) and move on. Flag it; never invent the answer or coach toward it. If these dominate, the input is too thin — suggest `prd`.
...
If the input is genuinely too thin to distill (e.g. "an app for hikers" with no surrounding context), stop and suggest `prd` (or sibling ceremony skill). This skill distills; it does not coach.
...
- **Spec-authored** companions are written by spec and live as **siblings of SPEC.md** (e.g., `glossary.md`, `patron-archetypes.md`). spec owns them and may edit them on update operations.
- **Adopted** companions are load-bearing artifacts written by an upstream skill that downstream still needs to read. spec references them into `companions:` by relative path but does NOT edit them (e.g., a `DESIGN.md` or `EXPERIENCE.md` from a UX run, an integration partner's API spec). The originating skill owns them.
...
- `companions:` array of `.md` files downstream MUST read alongside SPEC.md to have the full contract. Paths may point inside the spec folder (spec-authored companions like `glossary.md`) or outside it (adopted companions like `../ux/DESIGN.md` from the selected run's `02-plan/ux/` folder). The split between spec-authored and adopted is implicit by path; downstream treats both the same.
- `sources:` array of paths to files that were **fully absorbed** into the SPEC, with no remaining downstream value (e.g., a PRD whose every load-bearing claim is now in the kernel). Listed for audit and for spec to re-read on update. Downstream does NOT read these. Files that downstream still needs to read belong in `companions:`, not here.
