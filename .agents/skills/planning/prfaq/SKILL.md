...
Use `deep-recon` for market, competitive, domain, user-voice, or literature evidence; use `context7-mcp` for current named technology/API documentation; use `opensrc` for dependency behavior; use `gh` for primary repository and release evidence; and use `playwright-mcp` when live product behavior must be observed. Preserve source pointers and retrieval dates in the PRFAQ workspace.

Capture the runtime date before research and use it as the PRFAQ `as_of` boundary. Current market, competitor, feasibility, and version claims must be verified against sources available by that date; distinguish source publication/release dates from the date the agent retrieved them.
...
- Use `{run_dir}/02-plan/prfaq/` for durable output and `{run_dir}/02-plan/` for plan-input scanning. `{run_dir}` is the active initiative selected from `core.runs_root`.
...
1. **Run and resume detection:** Resolve one active run from `core.runs_root`; multiple active runs require explicit selection. A missing run is a blocker (the initiative-starting Create flow belongs to `product-brief` or `prd`). Check `{run_dir}/02-plan/prfaq/prfaq.md`; if it exists, read only the first 20 lines to extract `stage` and offer to resume. Never create a standalone PRFAQ file.

2. **Mode detection:**

- `--headless` / `-H`: Produce complete first-draft PRFAQ from provided inputs without interaction. Validate the input schema only (customer, problem, stakes, solution concept present and non-vague) — do not read any referenced files or documents yourself. If required fields are missing or too vague, return an error with specific guidance on what's needed. Fan out artifact analyzer and web researcher subagents in parallel (see Contextual Gathering below) to process all referenced materials, then create `{run_dir}/02-plan/prfaq/prfaq.md` using `./assets/prfaq-template.md` and route to `./references/press-release.md`. Register the draft before writing; an unresolved blocking finding stops the run.
...
**Graceful redirect:** If after 2-3 exchanges the user can't articulate a customer or problem, don't force it. Point them upstream to `brainstorming` to generate options.
...
2. **Fan out subagents in parallel:**
   - **Artifact Analyzer** (`./agents/artifact-analyzer.md`) — Scans `{run_dir}/02-plan/` and `{project_knowledge}` for relevant documents, plus any user-provided paths. Receives the product intent summary so it knows what's relevant.
...
**Create the output document** at `{run_dir}/02-plan/prfaq/prfaq.md` using `./assets/prfaq-template.md`. Register it as `02-plan/prfaq/prfaq.md` with status `draft` via `run_state.py add-artifact`, then write the frontmatter (populate `inputs` with any source documents used) and initial content captured during Ignition. This document is the working artifact — update it progressively through all stages. Keep stage review scratch under `02-plan/prfaq/reviews/` with a target slug/date.
