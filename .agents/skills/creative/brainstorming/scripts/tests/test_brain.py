...
import brain  # ty: ignore[unresolved-import] -- sibling is added to sys.path above
...
def test_resolve_detail_present(lib):
    ...
    detail = brain.resolve_detail(row, lib.parent)
    assert detail is not None
    assert "multi-step instructions" in detail
...
def test_html_writes_selection_page(lib, tmp_path):
    ...
    assert "workflow Method Brainstorming Selection" in doc
