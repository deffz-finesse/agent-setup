...
advanced-elicitation's pick_methods.py.)
...
def load_extra(file: Path) -> list[dict]:
    ...
    if not isinstance(data, list):
        raise TypeError("--extra must be a JSON array of objects")
    rows = []
    for item in data:
        if not isinstance(item, dict):
            raise TypeError(f"each --extra entry must be a JSON object, got: {item!r}")
        rows.append(
            {
...
<title>workflow Method Brainstorming Selection</title>
<script>
...
<header>
  <div class="hwrap">
  <div class="titlerow">
    <h1>workflow Method Brainstorming Selection</h1>
...
<footer>workflow Method &middot; Brainstorming</footer>
<script>
(function(){
