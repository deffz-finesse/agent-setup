...
- Resolve exactly one active run from `core.runs_root`; multiple active runs require explicit selection and a missing run blocks before writing.
- `output_dir` = `{run_dir}/00-discovery/`; derive a required `{topic_slug}` mechanically.
...
- `default_output_file` = `{output_dir}/{topic_slug}/problem-solution-{topic_slug}-{date}.md`

## Inputs

Register the draft and final document with `run_state.py add-artifact`.
...
- Use `{template_file}` as the structure when writing `{default_output_file}`. Register the draft and final document under the selected run with `run_state.py`.
...
<action>If the user will NOT run the optional Step 9 reflection, run: `{skill-root}/customize.toml` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
</step>
