...
Use `context7-mcp` for current test-framework and API expectations; use `opensrc` when a dependency's implementation affects whether a test is valid; use `playwright-mcp` for live browser behavior and selector verification; and use `gh` for primary repository, release, or issue evidence. Cite the source or version when it changes the review verdict.

Capture the runtime date before evaluating version-sensitive test behavior. Record the assessment `as_of` date, exact relevant versions, source publication/release dates, and retrieval dates when they affect the review verdict.
...
- Register each review, progress file, and validation report with `run_state.py add-artifact`; scope every repeated output by target slug.
- Require `{target_slug}` (story, epic, or suite identity); missing target identity blocks before writing.
- Register the target-scoped test-review report with `run_state.py`; never overwrite a report for another target/date.
