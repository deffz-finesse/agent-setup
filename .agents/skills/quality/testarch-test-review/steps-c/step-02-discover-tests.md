...
outputFile: '{test_artifacts}/test-review-{target_slug}.md'
---
...
> The legal reasons are `path does not exist` for a missing `review_files` entry, `file could not be parsed` for a read or parse failure, and `format not scorable by the ledger` for a changed artifact without registry criteria (such as a `.feature`, `.robot`, or `.http` file). Maestro flows are **not** in this set: the registry carries mobile rows (C4, C7, H1, H3, H4, H9, M8, L8), so a `.yaml` flow under a Maestro directory is discovered and scored like any other test file.
