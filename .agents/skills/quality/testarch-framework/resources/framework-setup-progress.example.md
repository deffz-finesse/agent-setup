...
- `tests/README.md` now documents that enforced rules come from the `Absolute`-severity rows of the `testarch-test-review` criteria registry (for example, the H1 hard-wait pattern and the H5 file-length ceiling), not from the hook itself, and that a rule can be turned off per project via `disabledRules` in `.tea/enforce-config.json` provided the reason is stated in the commit.
