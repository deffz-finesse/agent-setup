...
Use `context7-mcp` when acceptance tests depend on current framework, SDK, or API semantics; use `opensrc` when dependency internals determine observable behavior; use `playwright-mcp` for live browser journeys and selector evidence; and use `gh` for primary repository or issue evidence. Record load-bearing versions and sources in the ATDD checklist or its context artifacts.

Capture the runtime date before relying on version-sensitive acceptance behavior. Record exact framework/API versions, source pointers, and retrieval dates in the ATDD checklist or its context artifacts.
...
- Register each checklist, progress file, and validation report with `run_state.py add-artifact`; scope every repeated output by target slug.
- Require `{target_slug}` (normally the story key); missing target identity blocks before writing.
- Register every generated checklist, acceptance scaffold, and evidence file with `run_state.py`; use `draft` until validation and `current` after it.
