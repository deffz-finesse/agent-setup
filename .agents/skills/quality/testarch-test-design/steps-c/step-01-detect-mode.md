...
### B) File-Based Detection (workflow-Integrated)

If user intent is unclear:

- If `{run_dir}/03-slices/sprint-status.yaml` exists → **Epic-Level Mode**
...
Carry `epic_num`, `run_scope`, and `run_key` forward through every remaining step. Step 5 writes a target-scoped output under `{test_artifacts}` from the same `target_slug`, so a plan and its checkpoint always name the same run.
