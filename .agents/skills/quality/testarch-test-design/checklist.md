...
- [ ] Can proceed to `/testarch-atdd` workflow with P0 scenarios
- [ ] `/testarch-atdd` is a separate workflow and must be run explicitly (not auto-run)
...
- [ ] ❌ NO Follow-on Workflows section (workflow commands self-explanatory)
- [ ] ❌ NO Approval section
...
## workflow Handoff Validation (System-Level Mode Only)

- [ ] Handoff document generated at `{test_artifacts}/test-design-{target_slug}-handoff.md`
...
4. [ ] Run `/testarch-atdd` workflow to generate P0 tests (separate workflow; not auto-run)
