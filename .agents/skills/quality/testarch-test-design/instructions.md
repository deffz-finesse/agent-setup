...
This selects the progress checkpoint belonging to the run being resumed, reads its progress tracking frontmatter, and routes to the next incomplete step. Checkpoints are named `test-design-progress-{run_key}-{target_slug}.md` under `{test_artifacts}`, so each epic and the system-level run keep their own.
