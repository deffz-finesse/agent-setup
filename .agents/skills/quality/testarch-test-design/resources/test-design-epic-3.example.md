...
- Run `/testarch-atdd` explicitly for the four P0 scenarios.
- Run `/testarch-automate` for the remaining functional coverage after implementation exists.
- Run `/testarch-nfr` when evidence exists for all four NFR domains.
- Run `/testarch-trace` to map the approved oracle and apply the release gate.
...
**Generated by:** workflow TEA Agent, Test Architect Module
