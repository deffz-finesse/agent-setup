...
- `sprint_status` = `{run_dir}/03-slices/sprint-status.yaml`
- `report_file` = `{run_dir}/06-review/code-review-{target_slug}-{date}.md`; a missing target slug blocks before writing

All review evidence is persisted under the selected run's `06-review/` phase and
registered with `run_state.py add-artifact`. Story findings may also be appended to the
selected slice spec, but no standalone report root is permitted.
