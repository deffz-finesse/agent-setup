---
deferred_work_file: '{run_dir}/03-slices/backlog.md'
report_file: '{run_dir}/06-review/code-review-{target_slug}-{date}.md'
---
...
Before presenting findings, write the complete standalone report to `{report_file}` and register it as `current`; use the target slug/date so repeated reviews never overwrite one another. If `{spec_file}` exists and contains a Tasks/Subtasks section, append a `### Review Findings` subsection as a story-local mirror. Write all findings in this order:
