...
- **lenses** (optional) — one or more lens codes or names, however the caller expresses them: a spoken request, or a directive of the form `skill:review lenses=<code>[,<code>...]` (the form bmm's `doc_standards` uses). Default: every applicable lens (a full review).
...
When a review is run for an initiative, resolve exactly one active run from
`core.runs_root` and require an explicit `target_slug` (file, story, epic, or
other review subject). The default durable report is
`<run-dir>/06-review/review-<target_slug>-<date>.md`; create it without
overwriting an existing report, register it with `run_state.py add-artifact`, and keep any
reviewer scratch beside it. If no run is active, stop before writing rather
than falling back to a standalone report path.

1. **Resolve customization:** `{skill-root}/customize.toml`. On failure, read `{skill-root}/customize.toml` directly and use defaults. If a caller provides pre-resolved customization fields, honor them verbatim for those named fields and resolve only the remaining fields from your own `customize.toml`. Then execute each `{workflow.activation_steps_prepend}` entry in order, hold `{workflow.persistent_facts}` as standing context for the session, and treat `{workflow.review_guidance}` entries as standing review directives for every lens.
...
When `{workflow.report_path}` is set, write the report there only if it resolves
inside the selected run's `06-review/` phase; otherwise use the scoped default
above. Present the same report in chat after registration.
