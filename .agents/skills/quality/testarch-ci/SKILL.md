...
Use `context7-mcp` for current CI-provider, test-runner, and action documentation; use `opensrc` when dependency behavior affects pipeline commands or artifact collection; use `gh` for primary action, release, or issue evidence; and use `playwright-mcp` only when live browser execution is part of validation. Record version-sensitive choices in the CI artifact or configuration comments.
...
- Bind `{run_dir}` to the selected run and `{test_artifacts}` to `{run_dir}/05-testing` for CI evidence.
- Register each durable CI progress, validation, and evidence file with `run_state.py add-artifact`; scope repeated outputs by target slug.
- Require `{target_slug}` for any persisted CI report; repository CI configuration remains in its normal source-controlled location.
- Register durable CI notes/evidence with `run_state.py`; never create a second test-artifact root.
