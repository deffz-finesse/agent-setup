...
Use `context7-mcp` for current platform, framework, and service limits or guarantees; `opensrc` when implementation behavior affects an NFR assessment; `gh` for primary release and issue evidence; and `playwright-mcp` when live browser performance or behavior is part of the evidence. Record source, version, and retrieval date in the NFR artifact.

Capture the runtime date before assessing time-sensitive limits, guarantees, support windows, or performance behavior. Separate the assessment `as_of` date from source publication/release dates and retrieval dates.
...
- Register each assessment, progress file, and validation report with `run_state.py add-artifact`; scope every repeated output by target slug.
- Require `{target_slug}` (story, epic, or release identity); missing target identity blocks before writing.
- Register the scoped NFR report and evidence with `run_state.py` as `draft` then `current`.
