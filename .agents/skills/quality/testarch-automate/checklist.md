...
**Note:** workflow artifacts (story, tech-spec, PRD) are OPTIONAL - workflow can run without them
**Note:** `automate` generates tests; it does not run `/testarch-atdd` or `/testarch-test-review`. If ATDD outputs exist, use them as input and avoid duplicate coverage.
...
- [ ] **Note**: Absence of workflow artifacts does NOT halt workflow
...
**workflow-Integrated Mode (if story available):**
...
- [ ] Document includes execution mode (workflow-Integrated, Standalone, Auto-discover)
...
### With workflow Workflows (If Available - OPTIONAL)
...
- [ ] **Execution mode determined** (workflow-Integrated, Standalone, or Auto-discover)
...
### Issue: workflow artifacts not found

**Problem:** Story, tech-spec, or PRD files not found when variables are set.

**Resolution:**

- **automate does NOT require workflow artifacts** - they are OPTIONAL enhancements
- If files not found, switch to Standalone Mode automatically
- Analyze source code directly without workflow context
...
- Message: "Framework scaffolding required. Run `/testarch-framework` first."
...
- **automate is flexible:** Can work with or without workflow artifacts (story, tech-spec, PRD are OPTIONAL)
