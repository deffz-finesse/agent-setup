...
- [ ] If tests are missing, recommend `/testarch-atdd` (trace does not run it automatically)
- [ ] Test directory path is correct (`test_dir` variable)
- [ ] Story file is accessible (if using workflow mode)
...
- [ ] File created at `{test_artifacts}/traceability-{target_slug}-matrix.md`
...
- [ ] `links.trace_report_path` points to `traceability-{target_slug}-matrix.md`
...
- [ ] Coverage percentages extracted from the target-scoped traceability matrix (available from Phase 1)
- [ ] Coverage gaps extracted from the target-scoped traceability matrix (available from Phase 1)
- [ ] NFR status extracted from `nfr-{target_slug}-assessment.md` (if available)
- [ ] Security issues count extracted from `nfr-{target_slug}-assessment.md` (if available)
...
- [ ] If the target-scoped traceability matrix is missing, decision still possible with test results (but Phase 1 should provide it)
- [ ] If the target-scoped NFR assessment is missing, NFR validation marked as NOT ASSESSED
...
- If FAIL (either phase): Run `/testarch-atdd` for missing tests, fix issues, re-run `/testarch-trace`
