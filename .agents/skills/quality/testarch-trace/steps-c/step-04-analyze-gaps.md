...
outputFile: '{test_artifacts}/traceability-{target_slug}-matrix.md'
...
if (criticalGaps.length > 0) {
  recommendations.push({
    priority: 'URGENT',
    action: `Run /testarch-atdd for ${criticalGaps.length} P0 requirements`,
...
if (highGaps.length > 0) {
  recommendations.push({
    priority: 'HIGH',
    action: `Run /testarch-automate to expand coverage for ${highGaps.length} P1 requirements`,
...
recommendations.push({
  priority: 'LOW',
  action: 'Run /testarch-test-review to assess test quality',
  requirements: [],
});
