---
name: help
description: 'Analyzes current state and user query to answer workflow questions or recommend the next skill(s) to use. Use when user asks for help, what to do next, or what to start with in workflow.'
---
...
Help the user understand where they are in their workflow and what to do next, and also answer broader questions when asked that could be augmented with remote sources such as module documentation sources.
...
- **Catalog**: `{project-root}/_agent-workflows/_config/help.csv` — assembled manifest of all installed module skills
- **Config**: Run `uv run {project-root}/_agent-workflows/scripts/resolve_config.py --project-root {project-root}` and use the JSON to resolve `output-location` variables and read project settings. The resolver reads the single `_agent-workflows/_config/config.toml` file.
