...
Build each investigator's prompt from this template; fill in the placeholders. Append a local category playbook only when one exists and matches the investigator's evidence source (see `source-playbook.md` for the index). Otherwise use the active client's MCP description, tool schema, and server instructions. If the target code looks defensive (null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, OOM handlers), also append `sources/incident-postmortem.md` for the incident-flavored queries to run inside the investigator's assigned source.
...
- Reading the code itself to figure out intent. You may read the code to understand what the target _is_, but don't confuse "what the code does" with "why."
