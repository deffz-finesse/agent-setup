...
Transcript storage is client-specific. Use the active client's documented
session location or its context-transfer capability; never assume that a
transcript path from another client is present. Claude Code and Codex commonly
store local session records under their respective configuration roots, which
may be relocated by environment variables. Every line is one chat message when
the client uses JSONL transcripts.

1. Classify, then route. A request to resume one named prior session is still
   recall when the goal is to reconstruct context. Turning a repeated habit
   into a durable skill is a separate authoring task. A human-readable summary
   of the current work is also different. If the user already gave you a full
   state capsule (paths, branch, and change), use it and skip the mining.
