...
For automation, `-H <epic>` — an explicit epic in headless mode — is the stable orchestrator-facing interface. Pass the same number to `detect-epic --epic <N>` so the unfinished-story gate is script-backed (see Inputs). Epic auto-detection is a human convenience, not an automation contract: unflagged `detect-epic` returns the highest epic with _any_ `done` story, and stories-mode projects have no `sprint-status.yaml` to detect from.

## On Activation

Run these in order before the retrospective begins:

1. **Run prepend steps** — execute each entry in `{workflow.activation_steps_prepend}` in order.
2. **Load persistent facts** — treat every `{workflow.persistent_facts}` entry as standing context. `file:` entries are paths/globs under `{project-root}` whose contents load as facts; all others are literal facts.
3. **Greet and orient** (interactive only). Greet `the user`, name the epic you are about to retro, and optionally invite their going-in concerns ("anything you want weighted — a story that felt rushed, a risky interaction between two stories?"). Use any answer to focus the Phase 1–2 analysis; it directs attention but never becomes a finding without a source.
4. **Run append steps** — execute each entry in `{workflow.activation_steps_append}` in order.

Resolve exactly one active initiative from `core.runs_root` before selecting an
epic. Multiple active runs require explicit selection; a missing run blocks.
Durable retrospective output is `<run-dir>/08-retrospective/retro-<epic>-<date>.md`;
the sprint rollup is `<run-dir>/03-slices/sprint-status.yaml`, planning
inputs are `<run-dir>/02-plan/`, and review evidence is `<run-dir>/06-review/`.
Register the retrospective and any evidence files with `run_state.py add-artifact`; never use
standalone artifact roots.
...
| sprint status | `{run_dir}/03-slices/sprint-status.yaml` | epic detection + final status update |
| architecture / prd | `{run_dir}/02-plan/` | context for judging as-built vs intended |
| previous retro (optional) | `{run_dir}/08-retrospective/retro-*.md` | check whether last epic's actions landed |
| session logs (optional) | conversation/session records for the epic's stories | process lessons; record the gap when absent |

An epic reaches this skill in one of two shapes, and they are peers. **Sprint mode** reads `sprint-status.yaml`. **Stories mode** reads a slice/spec folder holding `SPEC.md`, an ordered `stories.yaml`, and story artifacts — the shape an unattended run leaves behind. Resolve which applies first: a named folder is stories mode whether or not sprint status exists; a named epic number is sprint mode; with neither, use the selected run's `03-slices/sprint-status.yaml`, and otherwise inspect `<run-dir>/03-slices/`. Ask the user which to retro when there is more than one, and never choose silently; headless, stop and require an explicit folder.
...
- **Diff-scope review** — do not reimplement review. Invoke **`review`** on the epic's diff for the code lenses (adversarial, edge-case, verification-gap), weighting the boundaries between stories, where no single session ever saw both sides. Fold its findings in. If `review` is unavailable, run those lenses inline over the diff on a narrowed scope and record the narrowing.
