...
# The authoritative action-item vocabulary, mirrored from sprint-planning's
...
def _load_yaml(path) -> tuple[Any, Any]:
    yaml = YAML(typ="rt")
...
def _emit(obj, code=0) -> NoReturn:
    sys.stdout.write(json.dumps(obj))
    sys.exit(code)


def _emit_error(message, code=1, restored=None) -> NoReturn:
...
def _match_action_items(entry, items):
    ...
    if isinstance(item_id, str):
        return [
            idx for idx, item in enumerate(items) if isinstance(item, Mapping) and item.get("id") == item_id
        ]
...
def _load_document(path, restored=None) -> tuple[Any, Any, Any]:
...
def cmd_update(args):
    ...
    if args.date is not None:
        try:
            parsed_date = datetime.strptime(args.date, DATE_FORMAT).replace(
                tzinfo=timezone.utc  # noqa: UP017 -- retain Python 3.10 compatibility
            )
        ...
        last_updated = datetime.now(timezone.utc).strftime(DATE_FORMAT)  # noqa: UP017
