...
3. **Sprint tracking**
   Check for `{run_dir}/03-slices/sprint-status.yaml`. If found, scan for stories with status `review`:
...
- If you have a commit or branch, check `{run_dir}/03-slices/` and `{run_dir}/04-implementation/` for a spec whose `baseline_commit` is an ancestor of that commit/branch (i.e., the spec describes work done on top of that baseline).
