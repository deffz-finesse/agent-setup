...
- `user_skill_level`
- exactly one active `run_dir` resolved from `core.runs_root` (multiple runs require explicit selection)
- `project_knowledge`
...
- `default_output_file` = `{run_dir}/02-plan/sprint-change-proposal-{target_slug}-{date}.md`; register it with `run_state.py`

## Input Files

| Input | Path | Load Strategy |
| ------- | ------ | --------------- |
| PRD | `{run_dir}/02-plan/prd.md` or its sharded children | FULL_LOAD |
| Epics | `{run_dir}/02-plan/epics/index.md` plus ordered shards | FULL_LOAD |
| Architecture | `{run_dir}/02-plan/architecture/` | FULL_LOAD |
| UX Design | `{run_dir}/02-plan/ux/` | FULL_LOAD |
| Spec | `{run_dir}/02-plan/spec/` | FULL_LOAD |
| Project Context | `AGENTS.md` in the affected repo (the `workflow:context` block) | FULL_LOAD |
...
1. **Read `AGENTS.md`** in the repo the change affects — the block between the `workflow:context` markers carries the policy, frozen paths, and conventions a course correction must respect.
...
<action>Report workflow completion to user with personalized message: "Correct Course workflow complete, the user!"</action>
<action>Remind user of success criteria and next steps for Developer agent</action>
<action>Run: `{skill-root}/customize.toml` — if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
</step>
