...
Present summary. Display the spec file path as a CWD-relative path (no leading `/`) so it is clickable in the terminal. If token count exceeded 1200 and user chose [K], include the token count and explain why it may be a problem.

After presenting the summary, display this note:

---

Before approving, you can open the spec file in an editor or ask me questions and tell me what to change. You can also use `advanced-elicitation` or `code-review`, ideally in another session to avoid context bloat.
...
Read fully and follow `[[snapshot:step-03-implement.md]]`
