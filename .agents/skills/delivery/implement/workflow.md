# Implement New Preview Workflow
...
A specification should target **one coherent commit** within roughly **400–1200 tokens**:

- **Single commit**: One atomic goal, even when the invariant requires a few tightly coupled files. Multi-goal means two or more changes that could be reviewed, tested, reverted, or committed independently. A branch or sprint may contain several slices; the slice is not the branch.
  - Split: "add dark mode toggle AND refactor auth to JWT AND build admin dashboard"
  - Don't split: "add validation and display errors" / "support drag-and-drop AND paste AND retry"
- **400–1200 tokens**: Enough context for one commit without turning a slice into a feature plan. Smaller is fine when the code and acceptance boundary are obvious.
...
Read fully and follow: `[[snapshot:step-01-clarify-and-route.md]]` to begin the workflow.
