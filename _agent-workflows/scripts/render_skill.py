...
# Make the repository package importable when this file is executed by path.
...
_SNAPSHOT_TOKEN = re.compile(r"\[\[snapshot:([A-Za-z0-9_./-]+\.md)\]\]")
...
def render(project_root: Path, skill_dir: Path) -> Path:
    ...
    if not (project_root / "_agent-workflows").is_dir():
        raise RenderError(f"project root does not contain _agent-workflows/: {project_root}")
